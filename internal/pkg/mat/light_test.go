...
func TestLightEyeBetweenLightAndSphere(t *testing.T) {
	...
	result := LightingPointLight(material, s, light, position, eyev, normalv, 1.0, NewLightData())
...
func TestLightingSamplesAreaLight(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		normalv := NewVector(tc.point.Elems[0], tc.point.Elems[1], tc.point.Elems[2])
		result := Lighting(sh.Material, sh, light, tc.point, eyev, normalv, 1.0, NewLightData())
		assert.InEpsilon(t, tc.color.Elems[0], result.Elems[0], Epsilon)
		assert.InEpsilon(t, tc.color.Elems[1], result.Elems[1], Epsilon)
		assert.InEpsilon(t, tc.color.Elems[2], result.Elems[2], Epsilon)
	}
	/*
