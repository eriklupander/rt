...
func TestNormalOnSphereAtNonAxial(t *testing.T) {
	...
	normalVector := NormalAt(s, NewPoint(nonAxial, nonAxial, nonAxial), nil)
...
func TestComputeNormalOnTranslatedSphere(t *testing.T) {
	...
	normalVector := NormalAt(s, NewPoint(0, 1.70711, -0.70711), nil)
...
func TestComputeNormalOnTransformedSphere(t *testing.T) {
	...
	s.SetTransform(m1)
	normalVector := NormalAt(s, NewPoint(0, math.Sqrt(2)/2, -math.Sqrt(2)/2), nil)
	assert.Equal(t, 0.0, normalVector.Get(0))
...
func TestColorAtWithMutuallyReflectiveSurfaces(t *testing.T) {
	w := NewWorld()
	w.Light = []Light{NewLight(NewPoint(0, 0, 0), NewColor(1, 1, 1))}
	lowerPlane := NewPlane()
